...
- 주소 검색 및 필지 선택
- 4종 정보를 보고서 형태로 한 화면에 순서대로 열람 (pills는 각 섹션으로 이동하는 앵커 내비게이션)
...
3. 선택한 필지 주소가 화면 상단에 고정되고, 그 아래 4개의 pills(앵커 내비게이션)가 나타난다.
4. 그 아래로 토지이용계획 · 토지대장 · 건축물대장 · 공시지가 4개 섹션이 보고서 형태로 순서대로 이어진다.
5. 사용자가 pills 중 하나를 누르면 해당 섹션으로 스크롤 이동한다. 스크롤을 직접 내려서 훑어볼 수도 있다.
...
- 입력: 사용자가 타이핑한 주소 문자열(자동 완성으로 띄우기)
- 동작: 검색 버튼 클릭 시 후보 목록 조회 → 목록에서 선택 (자동완성 방식 아님)
...
표시 항목: **용도지역, 건폐율, 용적률, 규제**

- 규제는 항목 수가 가변적이므로 목록 형태로 표시

---

## 5. 화면 설계

단일 페이지, **보고서 형태**로 4개 섹션을 한 화면에 이어서 보여준다. 
pills는 콘텐츠를 전환하는 탭이 아니라, 아래 섹션으로 스크롤 이동시키는 **앵커 내비게이션**이다.

섹션 노출 순서: **토지이용계획 조회 → 토지대장 → 건축물대장 → 공시지가 트래커**
...
│  선택된 필지 주소                                │
├─────────────────────────────────────────────────┤
│ (토지이용계획) (토지대장) (건축물대장) (공시지가) │  ← pills = 앵커 내비게이션
├─────────────────────────────────────────────────┤
│  ▣ 토지이용계획 조회 섹션                        │
│    용도지역 · 건폐율 · 용적률 · 규제              │
├─────────────────────────────────────────────────┤
...
- 4개 섹션은 필지 선택 시 **함께 조회를 시작** — 각 섹션이 서로 기다리지 않고 독립적으로 로딩 상태를 표시
...
| 2 | F-05 건폐율·용적률·규제를 어느 API/데이터로 얻을지 | 확인 필요 | 8-2장 2번 — `getLandUseAttr`엔 없음이 **확정됨**, 별도 출처 조사 필요 |
| 3 | 개별공시지가 필지 단위 조회 방법 및 5개년 조회 방식 | 확인 필요 | 8-2장 3번 |
| 4 | 도로명주소 · 지번주소 입력 모두 지원 여부 | 결정 필요 | 검색 API 지원 범위에 종속 (1번 확인 후 결정) |
| 5 | 차트 라이브러리 선정 | 결정 필요 | |
| 6 | 지도 표시 라이브러리 선정 | 결정 필요 | 좌표 획득(Geocoder)은 확인됨 — 지도를 실제로 그릴 라이브러리/위젯만 미정 |
