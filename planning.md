...
2. F-04 개별공시지가가 필지 단위로 조회되는지, 5개년을 1회/5회 중 어느 쪽으로 받는지 — 8-3에서 시도했으나 인증키 문제로 실제 호출까지 못 가서 여전히 미확인
...
| # | 항목 | 성격 | 비고 |
| --- | --- | --- | --- |
| 1 | 개별공시지가 필지 단위 조회 방법 및 5개년 조회 방식 | 년도별 조회 | 8-2장 2번 — 인증키 문제(11장 "즉시 조치 필요" 참조)로 실제 호출까지 못 가서 여전히 미확인 |
| 2 | 도로명주소 · 지번주소 입력 모두 지원 여부 | 도로명 주소만 지원 | 검색 API 지원 범위에 종속 |
| 3 | 차트 라이브러리 선정 | Recharts | |
| 4 | 지도 표시 라이브러리 선정 | Leaflet** (`react-leaflet`) | 좌표 획득(Geocoder)은 확인됨 — 지도를 실제로 그릴 라이브러리/위젯만 미정 |
...
| 6개 API 인증키 발급 단위 | VWorld 4종(토지·임야정보·토지이용계획·개별공시지가·Geocoder)은 `VWORLD_API_KEY` 통합, 건축물대장은 `BUILDING_API_KEY` 별도 (확정, 2026-07-27) — **단, 도로명주소는 별도 키 필요로 정정됨(아래 "즉시 조치 필요" 참조)** |
...
- ~~공공 API 인증키 발급~~ → 발급은 완료됐으나 **2가지 문제 발견**(2026-07-28, 8-3·11장 "즉시 조치 필요" 참조): 도로명주소는 juso.go.kr에서 별도 키를 새로 발급받아야 하고, VWorld 국가중점데이터 3종(토지·임야정보·개별공시지가·토지이용계획)은 추가 활용신청이 필요함
- 위 2가지 조치 완료 후 개별공시지가 API 재호출 테스트 → 11장 1번(필지단위·5개년 조회 방식) 확인
- 나머지 미결정 사항(11장) 결정
